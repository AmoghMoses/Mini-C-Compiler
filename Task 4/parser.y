...
array_random_access
	: array_identifier '[' array_random_access_breakup ']'
	| array_identifier '[' array_random_access_breakup ']' '[' array_random_access_breakup ']';
...
printf_statement
			: PRINTF '(' string_constant printf_identifier_list ')' ';' {print();};
...
mutable 
			...
			| array_random_access {if(!checkscope(curid)){printf("%s\n",curid);printf("Undeclared\n");exit(0);}
			                   	if(gettype(curid,0)=='i' || gettype(curid,1)== 'c')
			              		$$ = 1;
			              		else
			              		$$ = -1;
